<template>
    <el-container>
        <el-dialog title="亏损线路客户统计明细详情" :visible.sync="Visible" width="90%" :top="marginTop" @close="close">
    ...
    export default {
        ...
        methods: {
            ...
            searchList() {//查询
                let vm = this;
                vm.isLoading = true;
                vm.searchParam.actualDepartureStartTime= vm.startActualDepartureTime;//发车开始时间
                vm.searchParam.actualDepartureEndTime=vm.endActualDepartureTime;//发车结束时间
                vm.searchParam.customerCode=vm.currentRows.topCode;
                vm.searchParam.month=vm.month;
                vm.searchParam.regionCode=vm.regionCode;
                Http.get(Api.departSummary.lossLineCustomerStatistics,{ params : vm.searchParam }, result => {
