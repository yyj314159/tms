<template>
    <el-container>
        <el-dialog title="运单明细详情" :visible.sync="Visible" width="90%" :top="marginTop" @close="close">
            <el-main class="content-main">
                <el-table v-loading="isLoading" class="content-table" :data="searchResult.result"
                    ...
                    <el-table-column align="center" label="序列号" prop="waybill.departureNumber" min-width="180"
                    ...
                    <el-table-column align="center" label="小区" prop="waybill.dispatch.belongOrg.name" min-width="120"
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column align="center" label="线路名称" prop="waybill.dispatch.dispatchLine.lineName"
                    ...
                    <el-table-column align="center" label="操作发车时间" prop="waybill.operationDepartureTime" min-width="140"
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column align="center" label="实际发车时间" prop="waybill.actualDepartureTime" min-width="140"
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column align="center" label="到达签收时间" prop="waybill.confirmArrivalTime" min-width="140"
                                     show-overflow-tooltip></el-table-column>
                    ...
                    <el-table-column align="center" label="结算运费(应收)" prop="receiveConfirm.settlementAmount"
                                     min-width="120" show-overflow-tooltip=""></el-table-column>
                    <el-table-column align="center" label="运费调整调增(应收)" prop="receiveConfirm.feeAdjustAmount"
                    ...
                    <el-table-column align="center" label="结算时间(应收)" prop="receiveConfirm.billGenerationTime"
                                     min-width="140" show-overflow-tooltip></el-table-column>
                    <el-table-column align="center" label="应付运费(应付)"
                                     prop="waybill.dispatch.dispatchPrice.paidVehicleAmount" min-width="120"
                                     show-overflow-tooltip></el-table-column>
                    ...
                    <el-table-column align="center" label="时效扣款(应付)"
                                     ...
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column align="center" label="油价补差(应付)"
                                     prop="waybillNotarizePayableDeduct.oilPriceSupplement" min-width="120"
                                     show-overflow-tooltip></el-table-column>
                    ...
                    <el-table-column align="center" label="火灾险服务费(应付)" prop="waybill.dispatch.dispatchPrice.insuranceFee"
                                     min-width="120" show-overflow-tooltip></el-table-column>
                    <el-table-column align="center" label="打款手续费(应付)"
                                     prop="waybill.dispatch.dispatchPrice.handTransferFee"
                                     min-width="120" show-overflow-tooltip></el-table-column>
                    ...
                    <el-table-column align="center" label="计价方式(应收)" min-width="120" show-overflow-tooltip
                                     prop="waybill.dispatch.dispatchPrice.receivePriceType">
                        <template slot-scope="scope">
                    ...
                    <el-table-column align="center" label="计价方式(应付)" min-width="120" show-overflow-tooltip
                                     prop="waybill.dispatch.dispatchPrice.paidPriceType">
                        <template slot-scope="scope">
                            <dictionary-select-name option-name="LINE_PRICE_TYPE"
                    ...
                    <el-table-column align="center" label="车牌号"
                                     prop="waybill.dispatch.dispatchVehicle.vehicleLicenseNum" min-width="160"
                    ...
                    <el-table-column align="center" label="挂厢号"
                                     prop="waybill.dispatch.dispatchVehicle.trailerLicenseNum" min-width="120px"
                                     show-overflow-tooltip></el-table-column>
                    ...
                    <el-table-column align="center" label="始发地"
                                     prop="waybill.dispatch.dispatchLine.startStation.city.name" min-width="140"
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column align="center" label="目的地"
                                     prop="waybill.dispatch.dispatchLine.endStation.city.name" min-width="140"
                                     show-overflow-tooltip></el-table-column>
                    <el-table-column align="center" label="运单性质" prop="waybill.dispatch.dispatchLine.operationType"
                                     min-width="100">
                        <template slot-scope="scope">
    ...
    export default {
        ...
        data() {
            return {
                ...
                searchParam: {//查询参数
                    ...
                    regionCode: '',
                    customerCode: '',
                    kabanFlag: '',//增加卡板标识，区分查询条件中是否要区分卡板
                    lineCode: '',//应付异常报表 传线路编号
                    specification: '',//应付异常表表 传车型
                },
                searchResult: {//返回结果
                    totalCount: 0,
                    result: []
        ...
        methods: {
            ...
            searchList() {//查询
                ...
                vm.searchParam.regionCode = vm.regionCode;
                vm.searchParam.checkValue = vm.checkValue;
                vm.searchParam.customerCode = vm.customerCode;
                vm.searchParam.belongOrgCode = vm.currentRowed.code;
                vm.searchParam.kabanFlag = vm.kabanFlag;
                if (vm.orgCode) {
                ...
                if (vm.optionMenu === 'customerProfitChart') {
                    Http.get(Api.departSummary.customerProfitChartDetailDialog, {params: vm.searchParam}, result => {
                ...
                if (vm.optionMenu === 'receiveSettlementUnusual') {
                    Http.get(Api.departSummary.receiveSettlementUnusualDetailDialog, {params: vm.searchParam}, result => {
                        this.searchResult.result = result.result;
                        this.searchResult.totalCount = result.totalCount;
                        this.searchParam.currentPage = result.currentPage;
                        this.searchParam.pageSize = result.pageSize;
                ...
                if (vm.optionMenu === 'payableSettlementUnusual') {
                    if (!vm.searchParam.actualDepartureStartTime || !vm.searchParam.actualDepartureEndTime) {
                        throw vm.$message.warning("必须选择开始时间和结束时间!")
                    }
                    vm.searchParam.regionCode = vm.currentRowed.regionCode;
                    vm.searchParam.belongOrgCode = vm.currentRowed.belongOrgCode;
                    vm.searchParam.specification = vm.currentRowed.specification;
                    vm.searchParam.lineCode = vm.currentRowed.lineCode;
                    Http.get(Api.departSummary.payableSettlementUnusualDetailDialog, {params: vm.searchParam}, result => {
                        this.searchResult.result = result.result;
                        this.searchResult.totalCount = result.totalCount;
                        this.searchResult.currentPage = result.currentPage;
                        this.searchResult.pageSize = result.pageSize;
                        vm.isLoading = false;
                    }, () => {
                        vm.isLoading = false;
                    })
