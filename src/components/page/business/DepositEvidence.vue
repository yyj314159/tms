<template>
    <el-dialog title="保证金凭证维护" :visible.sync="visible" width="50%" @close="close">
        ...
        methods: {
            ...
            submit() {
                ...
                vm.$refs.voucherMaintainForm.validate((valid) => {
                    if (valid) {
                        Http.post(Api.deposit.voucherMaintain, vm.voucherMaintainForm, result => {
                            vm.$message.success('保证金凭证维护成功！');
                            vm.close();
                        });
                    } else {
