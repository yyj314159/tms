<template>
    <el-container>
                    ...
                    <el-col :span="6">
                        <el-form-item label="充值类型:">
                            <dictionary-select v-model="searchParam.businessType" style="display: block"
                                               option-name="WAYBILL_RECORD_BUSINESS_TYPE"></dictionary-select>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                        <el-form-item label="充值状态:">
                            <dictionary-select v-model="searchParam.rechargeStatus" style="display: block"
                                               option-name="RECHARGE_STATUS"></dictionary-select>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
        ...
        <el-main class="content-main">
            <el-button v-if="btnLoading" v-permission="'/waybillRefuelingRecord/export'" size="mini" type="primary"
            ...
            <el-table v-loading="isLoading" size="small" class="content-table" :data="searchResult.result"
                ...
                <el-table-column header-align="center" label="所属大区" prop="orgName"
                ...
                <el-table-column align="center" label="定点加油金额" prop="businessAmount" min-width="120"></el-table-column>
                <el-table-column align="center" label="充值类型" min-width="150" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="WAYBILL_RECORD_BUSINESS_TYPE"
                                                v-model="scope.row.businessType"></dictionary-select-name>
                    </template>
                </el-table-column>
                <el-table-column align="center" label="充值状态" min-width="120" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="RECHARGE_STATUS"
                                                v-model="scope.row.rechargeStatus"></dictionary-select-name>
                    </template>
                </el-table-column>
                <el-table-column align="center" label="供应商" prop="supplierName" min-width="180"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column header-align="center" label="承运方" prop="shipperName"
                                 min-width="200" show-overflow-tooltip></el-table-column>
                <el-table-column align="center" label="联系人" prop="linkMan" min-width="120"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" label="联系电话" prop="contact" min-width="150"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" label="司机姓名" prop="driverName" min-width="120"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" label="司机电话" prop="driverPhone" min-width="150"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column header-align="center" label="电子油卡卡号" prop="oilCardCode"
                                 min-width="140" show-overflow-tooltip></el-table-column>
                <el-table-column header-align="center" label="车牌号" prop="vehicleLicenseNum"
                                 min-width="140" show-overflow-tooltip></el-table-column>
                <el-table-column header-align="center" label="挂厢号码" prop="trailerLicenseNum"
                                 min-width="140" show-overflow-tooltip></el-table-column>
                <el-table-column header-align="center" label="车型" prop="specification"
                                 min-width="100"></el-table-column>
                <el-table-column align="center" label="实际发车时间" prop="actualDepartureTime" min-width="150"
                                 show-overflow-tooltip></el-table-column>
                <el-table-column align="center" label="充值时间" prop="createTime" min-width="150"
                                 show-overflow-tooltip></el-table-column>
            </el-table>
            <el-row>
            ...
            <el-row style="font-size:13px; font-weight: bold;">
                ...
                <span style="color:red; margin-right:20px;">定点加油金额：{{businessAmountValue}}元</span>
            </el-row>
        </el-main>
    </el-container>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            searchList() {//查询
                let vm = this;
                vm.isLoading = true;
                Http.get(Api.waybillRefuelingRecord.page, {params: vm.searchParam}, result => {
            ...
            exportData() {
                ...
                Http.get(Api.waybillRefuelingRecord.page, {params: searchExportParam}, response => {
                    let header = ['所属大区', '所属分区', '运单号', '定点加油金额', '充值类型', '充值状态', '供应商', '承运方',
                        '联系人', '联系电话', '司机姓名', '司机电话', '电子油卡卡号', '车牌号码', '挂厢号码', '车型', '实际发车时间','充值时间'];
                    let filterVal = ['orgName', 'belongOrgName', 'businessCode', 'businessAmount',
                        {
                            type: 'dictionary',
                            'code': 'WAYBILL_RECORD_BUSINESS_TYPE',
                            'name': 'businessType'
                        },
                        {
                            type: 'dictionary',
                            ...
                            'name': 'rechargeStatus'
                        }, 'supplierName', 'shipperName', 'linkMan', 'contact', 'driverName', 'driverPhone', 'oilCardCode', 'vehicleLicenseNum', 'trailerLicenseNum', 'specification','actualDepartureTime','createTime'
                    ];
                    Common.excel.downloadExl(response.result, header, filterVal, "充值记录导出");
                    vm.btnLoading = true;
                }, () => {
                    vm.btnLoading = true;
                })
