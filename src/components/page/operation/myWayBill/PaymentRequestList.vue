<template>
    <el-container>
                ...
                <el-row>
                    ...
                    <!--<el-col :span="6">
                        <el-form-item label="车牌号:">
                            <el-input v-model="searchParam.licenseNum"/>
                        </el-form-item>
                    </el-col>-->
                    <el-col :span="6">
                        <el-form-item label="车牌号:">
                    ...
                    <el-col :span="6">
                        <el-form-item label="是否承运方:" style="text-align: center;">
                            <el-switch v-model="searchParam.motorcade" active-color="#13ce66" inactive-color="#ff4949" active-text="是" inactive-text="否" @change="motorcadeChange"></el-switch>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                        <el-form-item label="是否开票:" style="text-align: center;">
                            <el-switch :disabled="!searchParam.motorcade" v-model="searchParam.ticket" active-color="#13ce66" inactive-color="#ff4949" active-text="是" inactive-text="否" @change="changeStatus"></el-switch>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row>
                    <el-col :span="6">
                        <el-form-item label="证件齐全:">
                            <el-select v-model="searchParam.papersCompleteFlag" placeholder="请选择"  style="display:block;">
                                <el-option
                    ...
                    <el-col v-if="!searchParam.ticket && !searchParam.motorcade" :span="6">
                        <el-form-item label="申请审核状态:">
                            <dictionary-select option-name="PAYMENT_APPLY_AUDIT_STATUS" :excludeOptions="['PASS']"  v-model="searchParam.applyForStatus" style="display: block;"></dictionary-select>
                        </el-form-item>
                    </el-col>

                    <el-col  v-else :span="6">
                        <el-form-item label="账单生成状态:">
                            <dictionary-select option-name="PAID_BILL_STATUS" v-model="searchParam.supplierBillStatus" style="display: block;"></dictionary-select>
                        </el-form-item>
                    </el-col>
                    <el-col  :span="6" v-if="searchParam.motorcade">
                        <!--<el-form-item label="到达结款周期:">
                            <el-select v-model="searchParam.settleAccountsCycleFlag" placeholder="请选择"  style="display:block;">
                                <el-option
                                    v-for="item in settleAccountsCycleFlag"
                                    :key="item.value"
                                    :label="item.label"
                                    :value="item.value">
                                </el-option>
                    ...
                    <el-col v-if="!searchParam.motorcade" :span="6">
                        <el-form-item label="收款人:">
                            <el-input v-model="searchParam.payee"></el-input>
                        </el-form-item>
                    </el-col>
                    <template v-if="!searchParam.motorcade">
                        <el-col :span="6">
                            <!--<el-form-item label="到达结款周期:">
                                <el-select v-model="searchParam.delayedPaymentFlag" placeholder="请选择"  style="display:block;">
                                    <el-option
                                        v-for="item in settleAccountsCycleFlag"
                                        :key="item.value"
                                        :label="item.label"
                            ...
                            <el-form-item label="到达结款周期:">
                                <dictionary-select v-model="searchParam.delayedPaymentFlag" style="display: block"
                                                   option-name="SETTLE_ACCOUNTS_CYCLE_FLAG"></dictionary-select>
                            </el-form-item>
                        </el-col>
                        <el-col :span="24">
        ...
        <el-main class="content-main">
            <el-button v-show="showCreatBill || checkBoxShow" v-permission="'/payableConfirm/generatedBills'"  type="primary" size="mini" icon="el-icon-tickets" :disabled="!multipleSelection.length>0" @click="showAccountList">生成账单</el-button>
            <el-table v-show="showCreatBill" v-loading="isLoading" size="small" class="content-table" :data="searchResult.result" element-loading-text="拼命加载中" border fit highlight-current-row stripe
                      @selection-change="handleSelectionChange" ref="multipleTable" :height="tableHeight" :style="{height:'auto'}">
                ...
                <el-table-column label="运单状态" prop="waybill.status" min-width="120" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="WAY_BILL_STATUS" :value="scope.row.waybill?scope.row.waybill.status:''"></dictionary-select-name>
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="客户全称" prop="waybill.dispatch.customer.name" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="大区" prop="waybill.dispatch.region.name" min-width="180" show-overflow-tooltip/>
                <el-table-column header-align="center" label="小区" prop="waybill.dispatch.belongOrg.name" min-width="180" show-overflow-tooltip/>
                <el-table-column header-align="center" label="线路" prop="waybill.dispatch.dispatchLine.lineName" min-width="240" show-overflow-tooltip/>
                <el-table-column header-align="center" label="车辆属性" min-width="120">
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="VEHICLE_OPERATION_MODE" v-if="scope.row.waybill" :value="scope.row.waybill.dispatch.dispatchVehicle.vehicleNature"/>
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="供应商" prop="waybill.dispatch.dispatchVehicle.supplier.name" min-width="200" show-overflow-tooltip/>
                <el-table-column header-align="center" label="承运商" prop="waybill.dispatch.dispatchVehicle.shipper.name" min-width="200" show-overflow-tooltip/>
                <el-table-column header-align="center" label="车牌号" prop="waybill.dispatch.dispatchVehicle.vehicleLicenseNum" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="实际发车时间" prop="waybill.actualDepartureTime" min-width="220" show-overflow-tooltip/>
                <el-table-column header-align="center" label="实际到达时间" prop="waybill.confirmArrivalTime" min-width="220" show-overflow-tooltip/>
                <el-table-column header-align="center" label="签收时间" prop="waybill.signTime" min-width="220" show-overflow-tooltip/>
                ...
                <el-table-column header-align="center" label="供应商账单生成人" prop="supplierBillGenerationUser" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="供应商账单生成时间" prop="supplierBillGenerationTime" min-width="220" show-overflow-tooltip/>
                <el-table-column header-align="center" label="收款人" prop="payee" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="收款银行" prop="openingBank" min-width="160" show-overflow-tooltip/>
                <el-table-column header-align="center" label="收款卡号" prop="dueBankCardNo" min-width="200" show-overflow-tooltip/>
                <el-table-column header-align="center" label="始发地" prop="waybill.dispatch.dispatchLine.startStation.name" min-width="200" show-overflow-tooltip/>
                <el-table-column header-align="center" label="目的地" prop="waybill.dispatch.dispatchLine.endStation.name" min-width="200" show-overflow-tooltip/>
                <el-table-column header-align="center" label="营运类型" prop="waybill.dispatch.dispatchLine.operationType" min-width="120">
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="LINE_OPERATION_TYPE" v-if="scope.row.waybill" :value="scope.row.waybill.dispatch.dispatchLine.operationType"/>
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="车长" prop="waybill.dispatch.dispatchVehicle.specification" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="主司机姓名" prop="waybill.dispatch.dispatchVehicle.driver.name" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="主司机电话" prop="waybill.dispatch.dispatchVehicle.driver.mobile" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="应付合同号" prop="waybill.dispatch.dispatchPrice.contractCode" min-width="160" show-overflow-tooltip/>
            </el-table>
            <el-table v-show="!showCreatBill" v-loading="isLoading" size="small" class="content-table" :data="searchResult.result"
                      @selection-change="handleSelectionChange" ref="multipleTable1"
                      element-loading-text="拼命加载中" border fit highlight-current-row stripe :height="tableHeight" :style="{height:'auto'}">
                <el-table-column type="selection" min-width="40" :selectable="selectable" show-overflow-tooltip/>
                <el-table-column header-align="center" label="运单号" min-width="130" fixed="left" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <a href="javascript:void(0)" style="color: #409EFF;" @click="showWayBill(scope.row)">{{scope.row.waybillCode}}</a>
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="客户全称" prop="customerName" min-width="140" show-overflow-tooltip/>
                <el-table-column header-align="center" label="车型" prop="specification" min-width="120" show-overflow-tooltip />
                <el-table-column header-align="center" label="车牌号码" prop="licenseNum" min-width="140" show-overflow-tooltip/>
                <el-table-column header-align="center" label="线路名称" prop="lineName" min-width="240" show-overflow-tooltip/>
                <el-table-column header-align="center" label="所属大区" prop="regionName" min-width="140" show-overflow-tooltip/>
                <el-table-column header-align="center" label="所属分区" prop="belongOrgName" min-width="140" show-overflow-tooltip/>
                <el-table-column header-align="center" label="供应商" prop="supplierName" min-width="240" show-overflow-tooltip/>
                <el-table-column v-if="searchParam.motorcade" header-align="center" label="承运方" prop="shipperName" min-width="240" show-overflow-tooltip/>
                <el-table-column v-if="searchParam.motorcade" header-align="center" label="账单状态" min-width="120" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="PAID_BILL_STATUS" :value="scope.row.billStatus"/>
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="实际发车时间" min-width="160" show-overflow-tooltip>
                    <template slot-scope="scope">
                        {{scope.row.departureTime ? Common.date.dateFormat(scope.row.departureTime, "YYYY-MM-DD HH:mm:ss ") : ''}}
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="到达时间" min-width="160" show-overflow-tooltip>
                    <template slot-scope="scope">
                        {{scope.row.confirmArrivalTime ? Common.date.dateFormat(scope.row.confirmArrivalTime, "YYYY-MM-DD HH:mm:ss ") : ''}}
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="运单签收时间" prop="signTime" min-width="160" show-overflow-tooltip/>
                <el-table-column header-align="center" label="回单号" prop="receiptCode" min-width="160" show-overflow-tooltip/>
                <el-table-column header-align="center" label="应付运费" prop="paidVehicleAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="调整金额" prop="adjustAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="结算金额" min-width="120" show-overflow-tooltip>
                    <template slot-scope="scope">
                        {{Common.util.numberFormat(scope.row.paidVehicleAmount, 2) + Common.util.numberFormat(scope.row.adjustAmount, 2)}}
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="质量扣款" prop="qualityDeductionsAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="油价补差" prop="oilPriceSupplement" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="其他扣款" prop="otherDeductionsAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="安全扣款" prop="securityDeductionsAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="质量奖励" prop="qualityAward" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="压车费"   prop="detentionCharge" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="火灾险服务费" prop="insuranceFee" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="打款手续费" prop="handTransferFee" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="实际应付" prop="settlementAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="预付打卡" prop="prepaidCardAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="货到打卡" prop="deliveryAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="定点加油金额" prop="refuelingAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="油卡金额" prop="oilCardAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="油卡押金" prop="oilCardDepositCost" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="油卡付款记录" min-width="120" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="PAYMENT_TYPE" v-model="scope.row.oilCardDeposit"/>
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="回单押金" prop="returnDepositCost" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="回单付款记录" min-width="120" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="PAYMENT_TYPE" v-model="scope.row.returnDeposit"/>
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="GPS使用费" prop="gpsCost" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="GPS押金" prop="gpsDepositCost" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="GPS付款记录" prop="paidVehicleAmount" min-width="120" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="PAYMENT_TYPE" v-model="scope.row.gpsDeposit"/>
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="尾款" prop="returnAmount" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="尾款付款记录" min-width="120" show-overflow-tooltip>
                    <template slot-scope="scope">
                        {{scope.row.remainAmountRefunds? "已付" : "未付"}}
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="账单生成人" prop="generatedBillsPeople" min-width="160" show-overflow-tooltip/>
                <el-table-column header-align="center" label="账单生成时间" prop="generatedBillsTime" min-width="160" show-overflow-tooltip/>
                <el-table-column header-align="center" label="申请付款人" prop="applyForPayment" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="申请付款时间" prop="applyForDate" min-width="160" show-overflow-tooltip/>
                <el-table-column header-align="center" label="审核人" prop="verifier" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="审核时间" prop="auditTime" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="申请审核状态" min-width="120" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <dictionary-select-name option-name="PAYMENT_APPLY_AUDIT_STATUS" :value="scope.row.applyForStatus"/>
                    </template>
                </el-table-column>
                <el-table-column header-align="center" label="审核意见" prop="remark" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="收款人" prop="payee" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="开户行" prop="openingBank" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="收款银行" prop="dueBank" min-width="120" show-overflow-tooltip/>
                <el-table-column header-align="center" label="收款银行卡号" prop="dueBankCardNo" min-width="160" show-overflow-tooltip/>
                <div style="display: inline-block;" v-if="!checkBoxShow">
                    <el-table-column header-align="center" label="操作" min-width="160" fixed="right">
                        <template slot-scope="scope">
                            <el-button :disabled="scope.row.payment || scope.row.applyForStatus === 'CHECK_PENDING' || scope.row.applyForStatus === 'PASS'"  type="text" size="mini" v-permission="'/supplierPayableBill/applyForBalancePayment'" icon="el-icon-tickets" @click="showApplyInfo(scope.row)">申请付款</el-button>
                            <el-button :disabled="scope.row.payment || scope.row.applyForStatus !== 'CHECK_PENDING'" type="text" size="mini" v-permission="'/supplierPayableBill/auditForBalancePayment'" icon="el-icon-tickets" @click="showAudit(scope.row)">审核</el-button>
                        </template>
                    </el-table-column>
                </div>
            </el-table>
                ...
                <el-row>
                    ...
                    <el-col :span="6">
                        <el-form-item label="预付打卡:">
                            ...
                            <b v-show="applyInfoForm.waybill.dispatch.dispatchPrice.prepaidCardAmount>0">
                                <span v-if="applyInfoForm.waybill.outInvitePrepaymentStatus===Constant.PAY_STATUS.PAID" style="color:green;">(已付)</span>
                    ...
                    <el-col :span="6">
                        <el-form-item label="定点加油:">
                            {{applyInfoForm.waybill.dispatch.dispatchPrice.actualRefuelingVolume}}升 (计{{applyInfoForm.waybill.dispatch.dispatchPrice.refuelingAmount}}元)
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    ...
                    <el-col :span="6">
                        <el-form-item label="货到打卡:">
                            ...
                            <b v-show="applyInfoForm.waybill.dispatch.dispatchPrice.deliveryAmount>0">
                                <span v-if="applyInfoForm.waybill.outInviteCodStatus===Constant.PAY_STATUS.PAID" style="color:green;">(已付)</span>
                                <span style="color:red;" v-else>(未付)</span>
                            </b>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                        <el-form-item label="回单押金:">
                            ...
                            <b v-show="applyInfoForm.waybill.dispatch.dispatchPrice.returnDeposit > 0">
                                <span v-if="applyInfoForm.waybill.supplierPayableBill && applyInfoForm.waybill.supplierPayableBill.returnDeposit===Constant.PAYMENT_TYPE.PAYMENT" style="color:green;">(已退)</span>
                ...
                <el-row>
                    <el-col :span="20" v-if="!applyInfoForm.delayedPayment">
                        <el-form-item label="申请原因：">
                            <el-input type="textarea" :rows="2" :maxlength="200" placeholder="最多200个字符！" v-model="applyInfoForm.applyRemark"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="24" style="font-weight: bold; font-size:16px; color:red; text-align: center;">申请付款金额： {{applyAmount}} 元 {{`( ${applyAmountStr} )`}}</el-col>
                </el-row>
                <el-table class="content-table" v-show="applyInfoForm.waybillPayableDeducts.waybillPayableDeductDetails.length>0" :data="applyInfoForm.waybillPayableDeducts.waybillPayableDeductDetails" element-loading-text="拼命加载中" size="mini" border fit highlight-current-row stripe style="width:95%;">
                    <el-table-column header-align="center" align="center" label="调整项" min-width="120">
                        <template slot-scope="scope">
                            <dictionary-select-name option-name="WAYBILL_ADJUSTMENT_TYPE" :value="scope.row.type"></dictionary-select-name>
                        </template>
                    </el-table-column>
                    <el-table-column header-align="center" align="center" label="金额" prop="chargeSum" min-width="120"></el-table-column>
                    <el-table-column header-align="center" align="center" label="原因" prop="remark" min-width="200"></el-table-column>
                </el-table>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button size="small" @click="showApplyDialog = false">取 消</el-button>
                <el-button :disabled="isSubmitBtnDisabled"  size="small" type="primary" @click="applyInfoSubmit">确 定</el-button>
            </span>
        </el-dialog>
        ...
        <el-dialog title="审核"  :visible.sync="showAuditDialog" width="90%" size="mini" @close="resetAuditForm" top="2vh">
            <el-form :model="auditForm" size="mini" style="padding-left: 35px;" label-width="120">
                ...
                <el-row>
                    ...
                    <el-col :span="6">
                        <el-form-item label="预付打卡:">
                            ...
                            <b v-show="auditForm.waybill.dispatch.dispatchPrice.prepaidCardAmount>0">
                                <span v-if="auditForm.waybill.outInvitePrepaymentStatus===Constant.PAY_STATUS.PAID" style="color:green;">(已付)</span>
                                <span style="color:red;" v-else>(未付)</span>
                            </b>
                        </el-form-item>
                    </el-col>
                    ...
                    <el-col :span="6">
                        <el-form-item label="定点加油:">
                            {{auditForm.waybill.dispatch.dispatchPrice.actualRefuelingVolume}}升 (计{{auditForm.waybill.dispatch.dispatchPrice.refuelingAmount}}元)
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    ...
                    <el-col :span="6">
                        <el-form-item label="货到打卡:">
                            ...
                            <b v-show="auditForm.waybill.dispatch.dispatchPrice.deliveryAmount>0">
                                <span v-if="auditForm.waybill.outInviteCodStatus===Constant.PAY_STATUS.PAID" style="color:green;">(已付)</span>
                                <span style="color:red;" v-else>(未付)</span>
                            </b>
                        </el-form-item>
                    </el-col>
                    <el-col :span="6">
                        <el-form-item label="回单押金:">
                            ...
                            <b v-show="auditForm.waybill.dispatch.dispatchPrice.returnDeposit > 0">
                                <span v-if="auditForm.waybill.supplierPayableBill && auditForm.waybill.supplierPayableBill.returnDeposit===Constant.PAYMENT_TYPE.PAYMENT" style="color:green;">(已退)</span>
                                <span style="color:red;" v-else>(未退)</span>
                            </b>
                        </el-form-item>
                    </el-col>
                ...
                <el-table class="content-table" v-show="auditForm.waybillPayableDeducts.waybillPayableDeductDetails.length>0" :data="auditForm.waybillPayableDeducts.waybillPayableDeductDetails" element-loading-text="拼命加载中" size="mini" border fit highlight-current-row stripe style="width:90%;">
                    <el-table-column header-align="center" align="center" label="调整项" min-width="120">
                        <template slot-scope="scope">
                            <dictionary-select-name option-name="WAYBILL_ADJUSTMENT_TYPE" :value="scope.row.type"></dictionary-select-name>
                        </template>
                ...
                <el-row>
                    <el-col :span="6">
                        <el-form-item label="审核：">
                            <el-switch v-model="auditForm.applyForStatus" active-color="#13ce66" inactive-color="#ff4949" :active-value="Constant.AUDIT_STATUS.PASS" :inactive-value="Constant.AUDIT_STATUS.NO_PASS" active-text="同意" inactive-text="拒绝"></el-switch>
                        </el-form-item>
                    </el-col>
                    <el-col :span="18" style="font-weight: bold; font-size:16px; color:red; text-align: center;">申请付款金额： {{applyAmountAudit}} 元 {{`( ${applyAmountAuditStr} )`}}</el-col>
                </el-row>
                <el-row>
                    <el-col :span="22">
                        <el-form-item label="备注：">
                            <el-input type="textarea" :rows="2" :maxlength="200" placeholder="最多200个字符！" v-model="auditForm.remark" style="width:90%;"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>
            <span slot="footer" class="dialog-footer">
                ...
                <el-button :disabled="isSubmitBtnDisabled" size="small" type="primary" @click="auditFormSubmit" >确 定</el-button>
            </span>
        </el-dialog>

        <!-- 对账单窗口 -->
        <el-dialog title="生成对账单" :visible.sync="showAccountDialog" width="80%" @close="resetInnerSearchResult" :top="marginTop">
            <el-tabs v-model="accountActiveName">
                <el-tab-pane label="运费明细" name="first">
                    <el-table class="content-table" :data="innerSearchResult.confirmList" element-loading-text="拼命加载中" size="mini" border fit highlight-current-row stripe height="320">
                        <el-table-column header-align="center" label="运单号" min-width="130" fixed="left"  show-overflow-tooltip>
                            <template slot-scope="scope">
                                <a href="javascript:void(0)" style="color: #409EFF;" @click="showWayBill(scope.row)">{{scope.row.waybillCode}}</a>
                            </template>
                        ...
                        <el-table-column header-align="center" label="供应商账单生成人" prop="supplierBillGenerationUser" min-width="130" show-overflow-tooltip/>
                        <el-table-column header-align="center" label="供应商账单生成时间" prop="supplierBillGenerationTime" min-width="220" show-overflow-tooltip/>
                        ...
                        <el-table-column header-align="center" label="始发地" prop="waybill.dispatch.dispatchLine.startStation.name" min-width="200" show-overflow-tooltip/>
                        <el-table-column header-align="center" label="目的地" prop="waybill.dispatch.dispatchLine.endStation.name" min-width="200" show-overflow-tooltip/>
                        <el-table-column header-align="center" label="营运类型" prop="waybill.dispatch.dispatchLine.operationType" min-width="120" show-overflow-tooltip>
                            <template slot-scope="scope">
                                <dictionary-select-name option-name="LINE_OPERATION_TYPE" v-if="scope.row.waybill" :value="scope.row.waybill.dispatch.dispatchLine.operationType"/>
                            </template>
                        </el-table-column>
                    ...
                    <p style="font-size:13px; font-weight: bold; line-height: 30px;">
                        ...
                        <span style="color:red; margin-right:20px;">油价补差：{{innerSearchResult.oilPriceSupplementSum}}元</span>
                        <span style="color:red; margin-right:20px;">其他扣款：{{innerSearchResult.otherDeductionsAmountSum}}元</span>
                ...
                <el-tab-pane label="加油记录" name="second">
                    <el-form size="mini" :model="oilSearchParam" label-width="120px">
                        <el-row>
                            <el-col :span="12">
                                <el-form-item label="加油时间:">
                                    <el-date-picker
                                        v-model="oilSearchParam.refuelingTime"
                                        :default-time="['00:00:00', '23:59:59']"
                                        style="width: 100%"
                                        type="daterange"
                                        value-format="yyyy-MM-dd HH:mm:ss"
                            ...
                            <el-col :span="12">
                                <el-form-item style="float: right" label-width="0">
                                    <el-button type="success" icon="el-icon-search" @click="oilSearchList" >查询</el-button>
                                    <el-button type="warning" class="fa fa-repeat" @click="resetOilSearchParam">重置</el-button>
                                </el-form-item>
                            </el-col>
                        </el-row>
                    </el-form>
                    <el-table class="content-table" :data="oilSearchResult.refuelingRecord" element-loading-text="拼命加载中" size="mini"
                              @selection-change="oilHandleSelectionChange" ref="oilMultipleTable"
                              border fit highlight-current-row stripe height="320">
                        <el-table-column type="selection" min-width="40"/>
                        <el-table-column align="center" label="车牌号" prop="vehicleLicenseNum" min-width="140" show-overflow-tooltip/>
                        <el-table-column align="center" label="车辆性质" min-width="120" show-overflow-tooltip>
                            <template slot-scope="scope">
                        ...
                        <el-table-column align="center" label="加油供应商" min-width="180" show-overflow-tooltip>
                            <template slot-scope="scope">
                                <span v-if="scope.row.gasStation"><dictionary-select-name option-name="GAS_STATION" v-model="scope.row.gasStation.supplier"/></span>
                            </template>
                        </el-table-column>
                        <el-table-column align="center" label="加油时间" prop="refuelingTime" min-width="160" show-overflow-tooltip/>
                    ...
                    <p style="font-size:13px; font-weight: bold; height: 30px; line-height: 30px;">
                        <span style="margin-right:20px; color:blue;">加油合计：{{ paidAmountSum || 0 }}元</span>
                    </p>
                </el-tab-pane>
            </el-tabs>
            <p style="font-size:18px; font-weight: bold; height: 30px; line-height: 30px;text-align: center">
                <span style="margin-right:20px; color:blue;">账单金额 = 运费合计 - 加油合计<span style="margin-left: 50px; color:red;">{{(innerSearchResult.returnAmountSum - (paidAmountSum || 0)).toFixed(2)}}元</span></span>
            </p>
            <span slot="footer" class="dialog-footer">
                <el-button size="small" @click="showAccountDialog = false">取 消</el-button>
                <el-button :disabled="isSubmitBtnDisabled" size="small" type="primary" @click="creatBill">确 定</el-button>
            </span>
        </el-dialog>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            motorcadeChange(val){
                if(!val){
                    this.searchParam.ticket = false;
                    this.searchParam.supplierBillStatus = '';
                }
                this.searchParam.delayedPaymentFlag = '';
                this.searchParam.settleAccountsCycleFlag = '';
            },
            searchList() {//查询
                let vm = this;
            ...
            creatBill(){
                ...
                vm.isSubmitBtnDisabled = true;
                Http.post(Api.payableConfirm.generatedBills, {'waybills':waybills,'refuelingRecords':ids}, result => {
            ...
            oilSearchList(){
                let vm = this;
                if(!vm.oilSearchParam.supplierId){
                    vm.$message.error("请选择供应商");
                    throw "请选择供应商"
                }
                if(vm.oilSearchParam.refuelingTime && vm.oilSearchParam.refuelingTime.length == 2){
                    vm.oilSearchParam.refuelingStartTime = vm.oilSearchParam.refuelingTime[0];
                    vm.oilSearchParam.refuelingEndTime = vm.oilSearchParam.refuelingTime[1];
                }
                Http.get(Api.payableConfirm.getRefuelingRecordSum,{ params : vm.oilSearchParam }, result => {
                    vm.oilSearchResult = result.result;
                })
            },
            resetOilSearchParam(){
                this.oilSearchParam = this.cloneModel.oilSearchParam
            },
