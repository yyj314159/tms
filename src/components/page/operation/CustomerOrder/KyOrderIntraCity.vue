    ...
    <el-container>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>跨越城配</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <el-main class="content-main">
            <el-button v-if="exportLoading" icon="el-icon-download" v-permission="'/kyOrderIntraCity/export'" type="primary" size="mini" style="margin-left: 10px;" @click="exportList">导出</el-button>
            <el-table v-loading="isLoading" size="small" class="content-table" :data="searchResult.result" element-loading-text="拼命加载中" border fit highlight-current-row stripe :height="tableHeight" :style="{height:'auto'}">
                ...
                <el-table-column align="center" label="操作" fixed="right" min-width="300">
                    <template slot-scope="scope">
                        <el-button v-permission="'/kyOrderIntraCity/driverPrice'" type="text" size="mini"  @click="driverPriceDialog(scope.row)" :disabled="!(scope.row.status === Constant.CUSTOMER_ORDER_STATUS.NEW_CREATE||scope.row.status === Constant.CUSTOMER_ORDER_STATUS.OFFER)"><i class="fa fa-file-text-o"></i>&nbsp;&nbsp;报价</el-button>
                        <el-button v-permission="'/kyOrderIntraCity/cancelDriverPrice'" type="text" size="mini"  @click="cancelDriverPrice(scope.row)" :disabled="!(scope.row.status === Constant.CUSTOMER_ORDER_STATUS.OFFER)"><i class="fa fa-file-text-o "></i>&nbsp;&nbsp;取消报价</el-button>
                        <el-button v-permission="'/kyOrderIntraCity/quotation'" type="text" size="mini"  @click="dialogKyOrderQuotationDetailDialog(scope.row)"><i class="fa fa-file-text-o"></i>报价详情</el-button>
                    </template>
                </el-table-column>
            </el-table>
    ...
    export default {
        ...
        methods: {
            ...
            searchList() {//查询
                let vm = this;
                vm.isLoading = true;
                Http.get(Api.kyOrderIntraCity.page,{ params : vm.searchParam }, result => {
            ...
            exportList(){
                ...
                axios.get(Api.kyOrderIntraCity.export, {params: vm.searchParam, responseType: 'arraybuffer'}).then((response) => {
                    let blob = new Blob([response.data], { type: 'application/x-xls' });
                    let link = document.createElement('a');
                    link.href = window.URL.createObjectURL(blob);
                    //配置下载的文件名
                    link.download = '跨越城配.xls';
                    link.click();
            ...
            dialogKyOrderQuotationDetailDialog(row) {
                let vm = this;
                Http.get(Api.kyOrderIntraCity.quotationDetail, {params: {'thirdSn': row.thirdSn}}, result => {
                    if (result.result) {
