    ...
    <el-dialog :title="title" :visible.sync="visible" width="95%" :top="marginTop" @close="close" append-to-body>
        <el-form size="small" :model="addVehicleForm" ref="addVehicleForm" label-width="95px">
            <el-row>
                ...
                <el-col :span="5">
                    <el-form-item label="需求时间:" prop="demandTime" :rules="validate.required('需求时间',true)">
                        <el-date-picker style="display:block; width:100%;" v-model="addVehicleForm.demandTime" value-format="yyyy-MM-dd HH:mm:ss" :picker-options="pickerOptions" type="datetime" placeholder="选择日期时间" @change="resetVehicleInfo"></el-date-picker>
                    </el-form-item>
                </el-col>
                <el-col :span="5">
                    <el-form-item label="业务员:"  prop="salesman" :rules="validate.required('业务员',addVehicleForm.businessType==='SCATTERED_BUSINESS_TYPE')">
                        <all-user-select style="display:block;"  v-model="addVehicleForm.salesman" @change-option="changeSalesman"></all-user-select>
                    </el-form-item>
                </el-col>
                <el-col :span="5">
                    <el-form-item label="请车员:"  prop="carInviter" :rules="validate.required('请车员',addVehicleForm.businessType==='SCATTERED_BUSINESS_TYPE')">
                        <all-user-select style="display:block;"  v-model="addVehicleForm.carInviter" @change-option="changeCarInviter"></all-user-select>
                    </el-form-item>
                </el-col>
                <el-col :span="4">
                    <el-form-item label="客户订单:">
                        <el-input  v-model="addVehicleForm.customerOrderCode"></el-input>
                    </el-form-item>
                </el-col>
            </el-row>
            ...
            <el-row>
                ...
                <el-col :span="5">
                    <el-form-item label="挂厢号码:" prop="dispatchVehicle.trailerCode" :rules="[{ required: addVehicleForm.dispatchVehicle.isTrailerLicenseNumRequired, message: '挂厢号码不能为空', trigger: 'blur'}]">
                        <vehicle-select ref="trailerLicenseNum" v-model="addVehicleForm.dispatchVehicle.trailerCode" @change-option="queryTrailerVehicleInfo"
            ...
            <el-row>
                <el-col :span="10"><span class="blue">应收信息：<i v-if="receiveUnitPrice>0" style="color:red; font-size:12px;">{{receiveUnitPrice}}{{priceUnit(addVehicleForm.dispatchPrice.receivePriceType)}}</i></span></el-col>
            ...
            <el-row>
                <el-col :span="5">
                    <el-form-item label="定点加油:" prop="dispatchPrice.refuelingVolume" :rules="[validate.plus]">
                        <el-input v-model="addVehicleForm.dispatchPrice.refuelingVolume" placeholder=" 升" disabled></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="5">
    ...
    export default {
        ...
        data() {
            return {
                ...
                title:`派车 ： 【${this.currentRow.order.dispatch.belongOrg.name}】 【${this.currentRow.order.dispatch.customer.name}】 【${this.currentRow.order.dispatch.dispatchLine.lineName}】 【出发站点：${this.currentRow.order.dispatch.dispatchLine.startStation.name}】`,
                ...
                //提交的表单
                addVehicleForm: {
                    belongOrgCode: this.currentRow.order.dispatch.belongOrg?this.currentRow.order.dispatch.belongOrg.code:'',
                    belongOrgName: this.currentRow.order.dispatch.belongOrg?this.currentRow.order.dispatch.belongOrg.name:'',
                    code: this.currentRow.order.dispatch.code,
                    lineCode: this.currentRow.order.dispatch.lineCode,
                    demandTime:  Common.date.dateFtt("yyyy-MM-dd hh:mm:ss",new Date()),
                    returnGoods: '',
                    salesman: "",
                    carInviter: this.currentRow.order.dispatch.businessType!=='SCATTERED_BUSINESS_TYPE'?`${this.$store.getters.user.name} ${this.$store.getters.user.username}`:'',
                    ...
                    remark: "",//备注
                    businessType: this.currentRow.order.dispatch.businessType//业务类型
                },
                dispatchVehicleCloneObj: null,
                dispatchPriceCloneObj: null,
                //默认供应商
                defaultSupplier: this.currentRow.order.dispatch.belongOrg?(this.currentRow.order.dispatch.belongOrg.outsideSupplier?this.currentRow.order.dispatch.belongOrg.outsideSupplier:{id:"",name:""}):{id:"",name:""},
                pickerOptions: {
                    disabledDate(time) {
        ...
        mounted() {
            ...
            this.queryRepayInfo(this.currentRow.order.dispatch.lineCode);
            this.init();
        },
        computed: {
            ...
            prepaidCardAmount:{
                get(){
                    return this.addVehicleForm.dispatchPrice.prepaidCardAmount = this.getPrepaidCardAmount(this.selfOutsourceVehicle,this.allDayVehicleData,this.addVehicleForm.dispatchVehicle.ticket,this.addVehicleForm.dispatchPrice.prepaidCardAmount);
                },
                set(val){
                    this.$emit('input',val);
                }
        ...
        methods: {
            init(){
                ...
                if(!_.isNil(vm.dispatchVehicle)){
                    console.log(vm.dispatchVehicle)
                    setTimeout(()=>{
                        vm.$refs.vehicleLicenseNum.setCurrentVehicle({
                            code: vm.dispatchVehicle.code,
                            licenseNumber: vm.dispatchVehicle.licenseNumber
                        });
                        ...
                        vm.addVehicleForm.dispatchVehicle.vehicleCode = vm.dispatchVehicle.code;
                        vm.queryVehicleInfo({'code':vm.dispatchVehicle.code,'licenseNumber':vm.dispatchVehicle.licenseNumber});
                    ...
                    if(!_.isNil(this.dispatchPrice)){
                            this.setPrice()
                    }
                    if(!_.isNil(this.driverInfo)){
                        this.setDriver()
                    }
                }
            },
            ...
            setPrice(){
                let vm = this
                vm.addVehicleForm.dispatchPrice.paidVehicleAmount = vm.dispatchPrice.paidVehicleAmount
                vm.addVehicleForm.dispatchPrice.oilCardAmount = vm.dispatchPrice.oilCardAmount
                vm.addVehicleForm.dispatchPrice.deliveryAmount = vm.dispatchPrice.deliveryAmount
                vm.addVehicleForm.dispatchPrice.prepaidCardAmount = vm.dispatchPrice.prepaidCardAmount
            },
            // 将抢单或报价司机注入
            setDriver(){
                let vm = this
                vm.$nextTick(() => {
                    vm.$refs.addVehicleFormDriverName.setCurrentDriver(this.driverInfo);
                    vm.driverObj = this.driverInfo;
                    vm.addVehicleForm.dispatchVehicle.driverId = this.driverInfo.id;
                    vm.addVehicleForm.dispatchVehicle.driverName = this.driverInfo.name;
                    vm.addVehicleForm.dispatchVehicle.driverPhone = this.driverInfo.mobile;
            ...
            queryVehicleInfo(obj) {
                ...
                if(!_.isNil(obj)){//选择已有车牌号
                    vm.resetVehicleInfo();
                    if(!vm.addVehicleForm.demandTime){
                        return vm.$message.warning('请先选择需求时间！')
                    }
                    ...
                    Http.get(Api.vehicle.queryByCode, {params: {'code': obj.code}}, result => {
                        ...
                        if(result.result.operationMode === vm.constant.VEHICLE_OPERATION_MODE.SELF_SUPPORT){//自营车
                            ...
                            //包天车辆判断
                            Http.get(Api.allDayVehicle.queryByVehicleCode, {params: {'vehicleCode': obj.code,'demandTime':vm.addVehicleForm.demandTime}}, response => {
            ...
            //计算预付打卡
            getPrepaidCardAmount(selfOutsourceVehicle,allDayVehicleData,ticket,prepaidCardAmount){
                if(selfOutsourceVehicle){//自营合作车无预付款
                    return '';
                }
                if(ticket){//开票的无预付款
                    return '';
            ...
            //计算回单付款
            getReturnAmount(vehicleNature,paidVehicleAmount,prepaidCardAmount,oilCardAmount,oilCardDeposit,returnDeposit,gpsDeposit,gpsFee,deliveryAmount){
                ...
                if(paidVehicleAmount >= 0 && paidVehicleAmount !== ''){
                    return (Common.util.numberFormat(paidVehicleAmount) - Common.util.numberFormat(prepaidCardAmount) - Common.util.numberFormat(oilCardAmount) -
                        ...
                        Common.util.numberFormat(gpsFee) - Common.util.numberFormat(deliveryAmount)).toFixed(1);
                }
                return ''
            },
            //关闭窗口
            ...
            //确定并提交form表单
            submit() {
                let vm = this,warn5 = '';//车辆或线路是否已派车
                vm.$refs.addVehicleForm.validate((valid) => {
                    if (valid) {
                        Http.post(Api.dispatch.alreadySend, vm.addVehicleForm, result => {
                            warn5 = !result.result?'':result.result;
                            if(!vm.addVehicleForm.dispatchVehicle.trailerCode && vm.addVehicleForm.dispatchVehicle.specification>=13.5){//未选择挂厢--非挂车
                            ...
                            if((vm.addVehicleForm.dispatchPrice.paidPriceType === Constant.LINE_PRICE_TYPE.NORMAL || !vm.payUnitPrice) &&_.isNil(vm.allDayVehicleData)){
                                ...
                                if(vm.common.util.numberFormat(vm.addVehicleForm.dispatchPrice.returnAmount) < 0){
                                    return vm.$message.warning('回单付款金额不能小于零！');
                                }
                            }
                            ...
                            let warn3 = vm.addVehicleForm.dispatchPrice.returnAmount && vm.addVehicleForm.dispatchPrice.paidVehicleAmount && vm.common.util.numberFormat(vm.addVehicleForm.dispatchPrice.returnAmount) < vm.common.util.numberFormat(vm.addVehicleForm.dispatchPrice.paidVehicleAmount) / 10 ? "回单付款金额小于应付金额的一成!  " : "";
                            ...
                            if (warn1 + warn2 + warn3 + warn4 + warn5) {
                                ...
                                vm.$confirm(h('div', newData), '提示', {
                                    ...
                                    let postData = vm.currentRow
                                    postData.order.dispatch = vm.addVehicleForm
                                    Http.post(Api.orderBid.dispatchOrderBid, postData, result => {
            ...
            changeSalesman(obj){
                let vm = this;
                if(!_.isNil(obj)){
                    vm.addVehicleForm.salesman = `${obj.name} ${obj.username}`;
                }else{
